...
const AllBookings = () => {
  ...
  const {
    ...
    queryKey: ["users"],
    queryFn: async () => {
      const res = await fetch("https://eveent-server.vercel.app/bookings", {
        headers: {
  ...
  const handleDelete = (id, hotelName) => {
    swal({
      ...
      if (willDelete) {
        fetch(`https://eveent-server.vercel.app/booking/${id}`, {
          method: "DELETE",
          // headers: {
          //     authorization: `bearer ${localStorage.getItem('accessToken')}`
          // }
  ...
  const searchBooking = () => {
    ...
    if (transformBookings.length !== 0 && searchBy === "User Name") {
      transformBookings = transformBookings?.filter((booking) =>
        booking.name.toLowerCase().includes(searchingValue.toLocaleLowerCase())
      );
    }
        ...
        <div>
          <div>
            ...
            >
              ...
              <option selected value="User Name">
                User Name
              </option>
                    ...
                    <tr key={booking?._id}>
                        ...
                        <div>
                          ...
                          <div className="mt-4 flex flex-col gap-3">
                            <div className="flex items-center">
                              <TbCurrencyTaka />
                              <span
                                onClick={() => handleCopy(booking?.price)}
                                className="text-lg font-bold cursor-copy"
                              >
                                {booking.price}
                              </span>
                            </div>
                              ...
                              <div className="mask mask-squircle w-12 h-12">
                                <img
                                  src={`${
                                    booking?.photoURL
                                      ? booking.photoURL
                                      : "https://www.whitechapelgallery.org/wp-content/uploads/2020/07/blank-head-profile-pic-for-a-man.jpg"
                                  }`}
                                  alt="userPhoto"
                                />
                              </div>
                      ...
                      <td>
                        <p className="badge badge-accent">
                          {booking?.status ? booking.status : "Not yet"}
                        </p>
                      </td>
                      <td>
                        <div className="flex flex-col gap-3">
                          ...
                          <button
                            onClick={() =>
                              handleDelete(booking?._id, booking?.hotelName)
                            }
