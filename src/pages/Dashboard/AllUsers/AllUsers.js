...
const AllUsers = () => {
  ...
  const {
    ...
    queryKey: ["users"],
    queryFn: async () => {
      const res = await fetch("https://eveent-server.vercel.app/users", {
        headers: {
  ...
  const handleMakeAdmin = (id, email) => {
    swal({
      ...
      if (willDelete) {
        fetch(
          `https://eveent-server.vercel.app/user/makeAdmin/${id}/${email}`,
          {
            method: "PUT",
            headers: {
              authorization: `bearer ${localStorage.getItem("accessToken")}`,
  ...
  const searchUsers = () => {
    ...
    transformUsers = transformUsers.filter((user) =>
      user.email.toLowerCase().includes(searchingValue.toLowerCase())
    );
  ...
  const selectPageHandler = (selectedPage) => {
    if (
      selectedPage >= 1 &&
      selectedPage <= Math.ceil(users?.length / numberOfElementPerPage)
    )
                  ...
                  <tr key={user?._id}>
                    ...
                    <th>
                      <div className="flex flex-col gap-3">
                        {!(
                          user?.role === "admin" || user?.role === "hotelAdmin"
                        ) && (
                          <button
                            onClick={() =>
