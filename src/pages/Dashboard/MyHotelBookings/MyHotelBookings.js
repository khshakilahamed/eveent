...
const MyHotelBookings = () => {
  ...
  const { data: bookings = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://eveent-server.vercel.app/hotel/bookings/${user?.email}`,
        {
          headers: {
  ...
  const searchBookings = () => {
    ...
    transformHotels = transformHotels.filter((booking) =>
      booking.name.toLowerCase().includes(searchingValue.toLowerCase())
    );
  ...
  const selectPageHandler = (selectedPage) => {
    if (
      selectedPage >= 1 &&
      selectedPage <=
        Math.ceil(searchBookings()?.length / numberOfElementPerPage)
    )
