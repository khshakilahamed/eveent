...
const SearchPage = () => {
  ...
  const { data: hotels, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://eveent-server.vercel.app/hotels/search?location=${searchLocation}`
      );
  ...
  const transFormItems = () => {
    ...
    if (searchItem) {
      sortedItems = sortedItems.filter((sortedItem) =>
        sortedItem.name.toLowerCase().includes(searchItem.toLowerCase())
      );
    }
  ...
  return (
    <div className="max-w-[1400px] px-10 sm:px-20 mx-auto my-14">
      ...
      {activeSearchModal && (
        <SearchModal setActiveSearchModal={setActiveSearchModal} />
      )}
    </div>
  );
};
